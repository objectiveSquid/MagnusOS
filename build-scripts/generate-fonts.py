...
def main() -> None:
    if len(sys.argv) != 3:
        print(
            "Usage: python3 generate-fonts.py <output_directory> <output_header_path>"
        )
        sys.exit(1)

    subprocess.run(
        f"{sys.executable} -m pip install pillow bitarray",
        stderr=subprocess.DEVNULL,
        stdout=subprocess.DEVNULL,
        shell=True,
    )
    from PIL import Image as PIL_Image
    import bitarray as IMPORT_bitarray

    global Image
    global bitarray
    Image = PIL_Image
    bitarray = IMPORT_bitarray

    generate_font_files(sys.argv[1])
    generate_header_file(sys.argv[1], sys.argv[2])
