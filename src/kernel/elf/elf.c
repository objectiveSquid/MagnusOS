    ...
    for (uint32_t i = 0; i < entryCount; i++) {
        ...
        case 7: // R_386_JUMP_SLOT
            ...
            if (symbol->shndx != SHN_UNDEF) {
                // defined inside the shared object itself
                *targetAddress = (uint32_t)loadBase + symbol->value;
                break;
            }

            const char *symbolName = (char *)(stringTab + symbol->name);
            void *symbolAddress = getExportFunction(symbolName);

            if (symbolAddress == NULL) {
                if (ELF32_ST_BIND(symbol->info) == STB_WEAK) {
                    *targetAddress = 0; // if its weak and we cant find a symbol it can be NULL
                    break;
                }
                return ELF_SYMBOL_LOOKUP_ERROR;
            }
            *targetAddress = (uint32_t)symbolAddress;
            break;
        default:
...
int applyRelocationsA(void *loadBase, void *symbolTab, void *stringTab, Elf32_RelA *table, uint32_t entryCount) {
    for (uint32_t i = 0; i < entryCount; i++) {
        uint32_t type = ELF32_GET_TYPE(table[i].info);
        uint32_t symIndex = ELF32_GET_SYMBOL(table[i].info);
        ...
        case 6: // R_386_GLOB_DAT
        case 7: /* R_386_JUMP_SLOT */
            if (!symbolTab || !stringTab)
                return ELF_SYMBOL_LOOKUP_ERROR;

            ELF_32BitSymbol *symbol = &((ELF_32BitSymbol *)symbolTab)[symIndex];

            if (symbol->shndx != SHN_UNDEF) {
                *targetAddress = (uint32_t)loadBase + symbol->value + (uint32_t)addend;
                break;
            }

            const char *symbolName = (char *)((uint8_t *)stringTab + symbol->name);
            void *symbolAddress = getExportFunction(symbolName);

            if (!symbolAddress) {
                if (ELF32_ST_BIND(symbol->info) == STB_WEAK) {
                    *targetAddress = (uint32_t)addend;
                    break;
                }
                return ELF_SYMBOL_LOOKUP_ERROR;
            }

            *targetAddress = (uint32_t)symbolAddress + (uint32_t)addend;
            break;
        default:
            return ELF_UNSUPPORTED_RELOCATION_TYPE;
        }
...
int handleDynamicSection(void *loadBase, Elf32_Phdr *dynamicProgramHeader) {
    ...
    void *pltRelocationTable = NULL; // can be Elf32_Rel or Elf32_RelA
    ...
    if (pltRelocationType == DT_RELA)
        pltRelocationTableEntrySize = sizeof(Elf32_RelA);
    else if (pltRelocationType == DT_REL)
    ...
    if (pltRelocationTable != NULL) {
        if (pltRelocationTableEntrySize == sizeof(Elf32_Rel))
            status = applyRelocations(loadBase, symbolTab, stringTab, (Elf32_Rel *)pltRelocationTable, pltRelocationTableSize / pltRelocationTableEntrySize);
        else
            status = applyRelocationsA(loadBase, symbolTab, stringTab, (Elf32_RelA *)pltRelocationTable, pltRelocationTableSize / pltRelocationTableEntrySize);
